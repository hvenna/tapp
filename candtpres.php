...
<?php
	...
	$padd=strtoupper($padd);
	$pattern = '/^[A-Z _-/.,:]+$/';
	//if(!preg_match($pattern,$padd))
	//{
	//	echo "<pre>We are sorry. The Name".$name." is  invalid Make sure your name contains no special symbols.</pre>";
	//	exit;
	//}
	//$pradd=strtoupper($pradd);
	//if(!preg_match($pattern,$pradd))
	//{
	...
	|| ($_FILES["photo"]["type"] == "image/png")))
	{
		...
		{
			echo "<pre> Error in the file: " . $_FILES["photo"]["error"] . "</pre><br>";
		}
	}
	else
	{
		echo "<pre>You have uploaded an Invalid photo. We accept the following extensions:
		.gif,.jpeg,.jpg,.pjpeg,.x-png,.png</pre>";
		exit;
	}
	$temp = explode(".", $_FILES["t10th"]["name"]);
	$extension = end($temp);
	
	if ((($_FILES["t10th"]["type"] == "image/gif")
	|| ($_FILES["t10th"]["type"] == "image/jpeg")
	|| ($_FILES["t10th"]["type"] == "image/jpg")
	|| ($_FILES["t10th"]["type"] == "image/pjpeg")
	|| ($_FILES["t10th"]["type"] == "image/x-png")
	|| ($_FILES["t10th"]["type"] == "image/png")))
	{
		if ($_FILES["t10th"]["error"] > 0)
		{
			echo "<pre> Error in the file: " . $_FILES["photo"]["error"] . "</pre><br>";
		}
	}
	...
	{
		echo "<pre>You have uploaded an Invalid X marks list. We accept the following extensions:
		.gif,.jpeg,.jpg,.pjpeg,.x-png,.png</pre>";
		exit;
	}
	$temp = explode(".", $_FILES["t12th"]["name"]);
	$extension = end($temp);
	
	if ((($_FILES["t12th"]["type"] == "image/gif")
	|| ($_FILES["t12th"]["type"] == "image/jpeg")
	|| ($_FILES["t12th"]["type"] == "image/jpg")
	|| ($_FILES["t12th"]["type"] == "image/pjpeg")
	|| ($_FILES["t12th"]["type"] == "image/x-png")
	|| ($_FILES["t12th"]["type"] == "image/png")))
	{
		if ($_FILES["t12th"]["error"] > 0)
		{
			echo "<pre> Error in the file: " . $_FILES["photo"]["error"] . "</pre><br>";
		}
	}
	...
	{
		echo "<pre>You have uploaded an Invalid XII marks list. We accept the following extensions:
		.gif,.jpeg,.jpg,.pjpeg,.x-png,.png</pre>";
		exit;
	}
	...
	//move uploaded files
	$path='C:\wamp\www\tapp\students\\'.$regd;
	$x=mkdir($path,0777);
	$path='./students';
	$photopath="$path/".$regd."/".$_FILES["photo"]["name"];
	$t10thpath="$path/".$regd."/".$_FILES["t10th"]["name"];
	$t12thpath="$path/".$regd."/".$_FILES["t12th"]["name"];
	if($photopath==$t10thpath||$photopath==$t12thpath||$t12thpath==$t10thpath)
	{
		echo "<pre> Yo can't upload the same document twice.</pre><br>";
		exit;
	}
	if (file_exists("students/".$regd."/". $_FILES["photo"]["name"]))
	{
		?><pre> <?php echo $_FILES["photo"]["name"] . " already exists. ";
		exit;?></pre><?php
	}
	else
	{	
		move_uploaded_file($_FILES["photo"]["tmp_name"],"students/".$regd."/". $_FILES["photo"]["name"]);
	}
	if (file_exists("students/".$regd."/". $_FILES["t10th"]["name"]))
	{
		?><pre> <?php echo $_FILES["t10th"]["name"] . " already exists. ";
		exit;?></pre><?php
	}
	else
	{	
		move_uploaded_file($_FILES["t10th"]["tmp_name"],"students/".$regd."/". $_FILES["t10th"]["name"]);
	}
	if (file_exists("students/".$regd."/". $_FILES["t12th"]["name"]))
	{
		?><pre> <?php echo $_FILES["t12th"]["name"] . " already exists. ";
		exit;?></pre><?php
	}
	else
	{	
		move_uploaded_file($_FILES["t12th"]["tmp_name"],"students/".$regd."/". $_FILES["t12th"]["name"]);
	}
	...
	//insert into persons
	$query="INSERT INTO `persons`(`Username`,`check`) VALUES ('$regd','$check')";
	...
	$query="INSERT INTO `student details`(`RegdNo`, `Name`, `Father`, `DOB`, `Email`, `PerAddress`,
	`preAddress`, `Mobile`, `gender`) VALUES ('$regd','$name','$fname','$DOB','$email','$padd',
	'$pradd','$mobile','$gender')";
	...
	$query="INSERT INTO `tenth info`(`RegdNo`, `Board`, `passout`, `Marks`, `total`, `percentage`,`school`) VALUES
	('$regd','$Xboard','$Xpass','$Xmks','$Xtotal','$Xper','$Xschool')";
	...
	$query="INSERT INTO `12th details`(`RegdNo`, `Board`, `passout`, `Marks`, `total`, `percentage`, `college`) VALUES 
	('$regd','$XIIboard','$XIIpass','$XIImks','$XIItotal','$XIIper','$XIIschool')";
	...
	$query="INSERT INTO `ugpg info`(`RegdNo`, `degree`, `board`, `passout`, `sem1`, `sem2`,
	`sem3`, `sem4`, `sem5`, `sem6`, `sem7`, `sem8`, `tsem1`, `tsem2`, `tsem3`, `tsem4`, `tsem5`, 
	`tsem6`, `tsem7`, `tsem8`, `marks`, `total`, `percentage`, `dept`,`backlog`) VALUES ('$regd','$degree',
	'$Dboard','$Dpass','$S1M','$S2M','$S3M','$S4M','$S5M','$S6M','$S7M','$S8M','$S1T','$S2T','$S3T','$S4T',
	'$S5T','$S6T','$S7T','$S8T','$mk','$tot','$Dper','$dept','$backlog')";
	 
	$result5=mysqli_query($con,$query);
	if (!$result5) {
	echo "<pre> ERROR : ".mysqli_error($con)."</pre>"; }
	//insert into uploads
	$query="INSERT INTO `uploads`(`RegdNo`, `pic`, `10th`, `12th`) VALUES ('$regd','$photopath','$t10thpath','$t12thpath')";
	...
	if ($result1 and $result2 and $result3 and $result4 and $result5 and $result6 ) {
		mysqli_commit($con);
		echo "<pre> YOUR REQUEST HAS BEEN TAKEN. YOU'LL GET AN EMAIL TO ".$email." FROM 
					THE ADMIN ONCE YOUR REQUEST HAS BEEN ACCEPTED. </pre>";
		echo "<pre> YOU WILL BE REDIRECTED TO A PAGE WHERE YOU CAN SEE YOUR FILLED DETAILS. </pre>";
		$_SESSION['Next']=$check;
		?><html>
		<script>
			alert("Registartion Successful. Pleae wait . . You are being redirected.");
			window.location.assign("./abc.php");
		</script>
		</html><?php 
	} 
	else {        
		mysqli_rollback($con);
		$files = glob("students/".$regd."/*"); // get all file names
		foreach($files as $file)
		{ // iterate files
			unlink($file); // delete file
		}
		rmdir("students/".$regd);
		echo "<pre>Error in Registartion. Please try Again.</pre>";
	}
